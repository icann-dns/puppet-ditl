  ...
  on_supported_os.each do |os, facts|
    context 'on #{os}' do
      ...
      describe 'check default config' do
        ...
        it do
          is_expected.to contain_file('/usr/local/bin/ditl_upload')
              .with(
                ensure: 'present',
                mode: '0755',
              ).with_content(
                ...
                /-i \/root\/.ssh\/oarc_id_dsa/
              ).with_content(
                /dns-oarc@capture.ditl.dns-oarc.net/
              ).with_content(
                /ls \$\{PCAP_DIR\}\/\*\.\{bz2,xz\}/
              ).without_content(
                /ssh-keygen -f/
        ...
        it do
          is_expected.to contain_file('/root/.ssh/oarc_id_dsa')
              .with(
                ensure: 'present',
                mode: '0600',
        ...
        it do
          is_expected.to contain_cron('ditl_upload')
              .with(
                ensure: 'present',
      ...
      describe 'Change Defaults' do
        ...
        context 'upload_user' do
          ...
          it do
            is_expected.to contain_file('/usr/local/bin/ditl_upload')
              .with_content(/foobar@capture.ditl.dns-oarc.net/)
          end
        end
        context 'enabled' do
        ...
        context 'pattern' do
          ...
          it do
            is_expected.to contain_file('/usr/local/bin/ditl_upload')
              .with_content(/ls \$\{PCAP_DIR\}\/foobar\.\*/)
          end
        end
        ...
        context 'clean_known_hosts' do
          ...
          it do
            is_expected.to contain_file('/usr/local/bin/ditl_upload')
              .with_content(
                /ssh-keygen -f \"\/root\/\.ssh\/known_hosts\" -R capture\.ditl\.dns-oarc\.net  >\/dev\/null 2>\&1/
              )
          end
        end
      end
